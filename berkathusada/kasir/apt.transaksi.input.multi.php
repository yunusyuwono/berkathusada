            ...
            <div class="col-12 mt-1 mb-2">
					...
					<div class="card-header bg-primary text-white">
						<div class="row">
							<div class="col-md-6">
								<nav aria-label="breadcrumb">
									...
									if(mysqli_num_rows($gntrk)==0)
									{
										...
										$_SESSION['nofaktur']=$nofaktur;
									}
									else
									{
										$cts=mysqli_fetch_array(mysqli_query($kon,"select * from apotek_trk where idkasir ='$_SESSION[iduser]' order by id desc limit 1"));
										if($cts['status']=='selesai')
										{
											$nofaktur="TBHA".date('Y').sprintf("%05s",$cts['id']+1);
											$tglfaktur=date('Y-m-d');
											$sesikasir=$_SESSION['iduser'];
											mysqli_query($kon,"insert into apotek_trk (nofaktur,tglfaktur,idkasir,status) values ('$nofaktur','$tglfaktur','$sesikasir','proses')");
											$ctsk=mysqli_fetch_array(mysqli_query($kon,"select * from apotek_trk where idkasir='$_SESSION[iduser]' and status='proses' order by id desc limit 1"));
											$_SESSION['nofaktur']=$ctsk['nofaktur'];
										}
										elseif ($cts['status']=='proses') 
										{
											if($cts['idkasir']==$_SESSION['iduser'])
											{
												$_SESSION['nofaktur']=$cts['nofaktur'];
											}
											else
											{
												$nofaktur="TBHA".date('Y').sprintf("%05s",$cts['id']+1);
									...
									$nofaktur=$_SESSION['nofaktur'];
										/*
										if(isset($_SESSION['nofaktur']))
										{
											$nofaktur=$_SESSION['nofaktur'];
										}
										else
										{
											$lid=mysqli_fetch_array(mysqli_query($kon,"SELECT max(id) as lastid from apotek_trk"));
											$nofaktur="TBHA".date('Y').sprintf("%05s",($lid['lastid']+1));
											$tglfaktur=date('Y-m-d');
											$sesikasir=$_SESSION['iduser'];
											mysqli_query($kon,"insert into apotek_trk (nofaktur,tglfaktur,idkasir,status) values ('$nofaktur','$tglfaktur','$sesikasir','proses')");
											$_SESSION['nofaktur']=$nofaktur;
										...
										*/
									$gf=mysqli_fetch_array(mysqli_query($kon,"SELECT * from apotek_trk where nofaktur='$nofaktur'"));
									?>
									<ol class="breadcrumb">
							...
							<div class="col-md-6" align="right">
								<div class="btn-group">
								<a class="btn btn-lg text-white" href="apt.transaksi"><i class="fas fa-caret-left"></i> Kembali</a>
								</div>
							</div>
						</div>
					</div>
								...
								<div class="col-lg-12">
									<div class="row">
										<div class="form-group col-md-6">
											...
											<input type="date" class="form-control" name="tglf" id="tglf" value="<?=$gf['tglfaktur'];?>">
										</div>
										<div class="col-md-3">
											<br>
											<a class="btn btn-sm btn-success" onclick="settglfaktur('<?=$nofaktur;?>')">Atur Ulang Tanggal</a>
										</div>
				...
				<div class="card card-body">
					...
					<div class="btn-group btn-block">
						...
						<a class="btn btn-outline-primary" href="apt.transaksi.selesai?nofaktur=<?=$nofaktur;?>&cetak" target="_blank"><i class="fas fa-print"></i> Selesai dan Cetak Transaksi</a>
					</div>
				</div>
				</div>
