        ...
        <div class="card">
            ...
            <div class="card-body mt-2">
                ...
                if(isset($_POST['kirim']))
                {
                    ...
                    $brg=mysqli_fetch_array(mysqli_query($kon,"select * from barang where kode='$kode'"));
                    $cos=mysqli_fetch_array(mysqli_query($kon,"select * from so_cek where kode='$so' and kodebarang='$kode'"));
                    $selisih=$brg['jml_apotek']-$jmlr;
                    if(!empty($cos['stokreal']))
                    {
                    $simpan=mysqli_query($kon,"update so_cek set stokreal='$jmlr',ket='$ket',selisih='$selisih' where id='$cos[id]'");                        
                    }
                    else
                    {
                    $simpan=mysqli_query($kon,"insert into so_cek (kode,kodebarang,stokreal,selisih,ket) values('$so','$kode','$jmlr','$selisih','$ket')");
                    }

                    if($simpan)
                    {
                        mysqli_query($kon,"UPDATE barang set jml_apotek='$jmlr' where kode='$kode'");
                    ?>
                    ...
                    <table class="table table-striped table-hover table-bordered table-responsive">
                        ...
                        while ($b=mysqli_fetch_array($barang)) {
                            ...
                            <tr>
                                ...
                                <td><?=$b['kode'];?></td>
                                <td><?=$b['nama'];?></td>
                                <td><?=$jumlah+(isset($cso['selisih'])?$cso['selisih']:0);?></td>
                                <td><?=$b['ed'];?></td>
                                <?php 
