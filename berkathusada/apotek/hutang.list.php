...
if(!isset($_GET['cari']))
{
	$trkapotek=mysqli_query($kon,"select * from apotek_trk where status='selesai' and status_bayar='hutang' order by entri desc");
...
{
	$trkapotek=mysqli_query($kon,"select * from apotek_trk where status='selesai' and status_bayar='hutang' and nofaktur like '%$cari%' or status='selesai' and tglfaktur like '%$cari%' and status_bayar='hutang'  order by entri desc");
	if(mysqli_num_rows($trkapotek)==0)
	{
