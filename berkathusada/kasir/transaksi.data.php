<table class="table table-responsive table-striped table-hover table-bordered" width="100%">
	...
	<div class="container p-1 pb-2">
		<div class="btn-group">
		<a class="btn btn-sm btn-light" data-bs-toggle="modal" data-bs-target="#ptrk" title="Pilih Pasien"><i class="fas fa-plus-circle"></i> Pilih Pasien
			<?php
		...
		<div class="btn-group">
			...
			?>
			<a class="btn btn-sm <?=$bon;?>" href="transaksi.ppn?nofaktur=<?=$nofaktur;?>&ppn=on" title="PPN ON"> PPN ON</a>

			<?php 
			if(!$gn['ppn']=='')
				{
			?>
			<a class="btn btn-sm <?=$bof;?>" href="transaksi.ppn?nofaktur=<?=$nofaktur;?>&ppn=off" title="PPN OFF"> PPN OFF</a>
			<?php 
				}
			?>
		</div>
	</div>
<form method="post" action="transaksi.bayar.php">
<table class="table table-responsive table-striped table-hover table-bordered" width="100%">
				...
				<form method="post" action="transaksi.pelanggan">
					<div class="form-group">
						...
						<select class="form-control" name="plg">
							...
							{
								?>
								<option value="<?=$p['kode'];?>" <?php echo 
								isset($gp['kodeplg'])?($p['kode']==$gp['kodeplg'])?"selected":'':''; ?> ><?=$p['nama'];?></option>
								<?php
							}
							?>	
						</select>
